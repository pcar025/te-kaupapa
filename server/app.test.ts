...
describe('authenticated application shell API', () => {
  ...
  it('exposes only the minimum assigned PHQ escalation record to its resolved supervisor', async () => {
    ...
    const list = await app.inject({ method: 'GET', url: '/api/phq9-escalations/assigned', headers: { cookie: 'test_session=phq-supervisor' } })
    ...
    expect((await app.inject({ method: 'GET', url: detailUrl, headers: { cookie: 'test_session=phq-other-supervisor' } })).statusCode).toBe(404)
    expect((await app.inject({ method: 'GET', url: detailUrl, headers: { cookie: 'test_session=phq-foreign-supervisor' } })).statusCode).toBe(404)
