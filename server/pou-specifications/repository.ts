...
export class PostgresOrganisationPouSpecificationRepository {
  ...
  async resolveActivePin(organisationId: string, safetyPin: AssessmentStartPin): Promise<PouSpecificationStartPin> {
    const rows = await this.db.select({ activation: schema.organisationPouSpecificationActivations, specification: schema.organisationPouSpecificationVersions, guidance: schema.conversationGuidanceProjections, review: schema.pouReviewProjections, link: schema.organisationPouSafetySpecificationLinks })
      ...
      .where(and(eq(schema.organisationPouSpecificationActivations.organisationId, organisationId), eq(schema.organisationPouSpecificationActivations.pouId, 'whakapapa'), sql`${schema.organisationPouSpecificationActivations.deactivatedAt} is null`)).limit(1)
    const row = rows[0]
    if (!row || row.specification.approvalStatus !== 'approved_for_pilot' || !row.specification.approvedForPilotBy || !row.specification.approvedForPilotAt) throw new PouSpecificationUnavailableError('No active approved Whakapapa organisation specification is available.')
