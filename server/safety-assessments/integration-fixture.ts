...
export async function withPhase5BTestContext<T>(body: (context: any) => Promise<T>, options: { pouId?: 'whakapapa' | 'kaitiakitanga' } = {}): Promise<T> {
  ...
  const pouId = options.pouId ?? 'whakapapa'
  ...
  return withMigratedTestDatabase(async (connection: DatabaseConnection) => {
    ...
    await connection.db.insert(schema.organisations).values({ id: organisationId, slug: actor.organisation.slug, name: actor.organisation.name }); await connection.db.insert(schema.appUsers).values({ id: userId, organisationId, email: `${userId}@example.invalid`, displayName: actor.displayName }); await connection.db.insert(schema.workflowSessions).values({ id: workflowId, organisationId, kaimahiUserId: userId, reference: `TK-${workflowId.slice(0, 8)}`, status: 'in_progress', currentStage: 'pou-overview', currentPouId: pouId, version: 2 }); await connection.db.insert(schema.workflowPouCheckpoints).values(historicPouOrder.map((checkpointPouId, index) => ({ workflowSessionId: workflowId, organisationId, pouId: checkpointPouId, ordinal: index + 1 })))
    const [storedSpec] = await connection.db.insert(schema.safetySpecificationVersions).values({ organisationId, specificationCode: spec.specificationCode, specificationVersion: spec.specificationVersion, pouId, approvalStatus: spec.approvalStatus, contentHash: specHash, ruleManifestHash: contentHash(projection.rules), specification: spec, sourceDocumentCode: spec.sourceDocumentCode, sourceDocumentStatus: spec.sourceDocumentStatus, sourceReference: spec.sourceReference, sourceDocumentHash: spec.sourceDocumentHash, derivedAt: now, approvedForPilotBy: userId, approvedForPilotAt: now }).returning(); const [storedProjection] = await connection.db.insert(schema.providerAssessmentProjections).values({ organisationId, pouId, specificationId: storedSpec!.id, projectionCode: projection.projectionCode, projectionVersion: projection.projectionVersion, projectionHash, provider: 'elevenlabs', providerAgentReference: 'agent-test', providerBranchReference: 'branch-test', providerEnvironment: 'test', projection }).returning(); await connection.db.insert(schema.safetySpecificationActivations).values({ organisationId, pouId, specificationId: storedSpec!.id, projectionId: storedProjection!.id, activatedByUserId: userId, activatedAt: now })
    ...
    await connection.db.execute(sql`alter table workflow_pou_review_criterion_snapshot disable trigger workflow_pou_review_criterion_snapshot_immutable`)
    await connection.db.execute(sql`alter table workflow_pou_review_criterion_snapshot disable trigger workflow_pou_review_criterion_snapshot_set_on_snapshot`)
    ...
    try {
      ...
      await connection.db.execute(sql`alter table workflow_pou_review_criterion_snapshot enable trigger workflow_pou_review_criterion_snapshot_immutable`)
      await connection.db.execute(sql`alter table workflow_pou_review_criterion_snapshot enable trigger workflow_pou_review_criterion_snapshot_set_on_snapshot`)
