...
function prompt(projection: ProviderAssessmentProjection): string {
  ...
  return `Assess only explicit current-conversation evidence using this immutable Te Kaupapa Pou assessment projection: ${JSON.stringify(rules)}. Return every rule exactly once. Do not infer from silence or identity. candidateConcernLevel is always null. A possible_concern requires at least one approved matchedConcernIndicatorCode, no missingInformationCodes, and null applicabilityReasonCode. insufficient_information requires at least one approved missingInformationCode and null applicabilityReasonCode. not_applicable requires an approved applicabilityReasonCode. no_candidate_concern requires at least one approved matchedProtectiveIndicatorCode and an evidenceTurnId showing adequate exploration; it has no concern code and null applicabilityReasonCode. evidenceTurnIds may contain only supplied stable turn IDs, no duplicates, and no more than eight IDs; use them for direct supporting source material only. Do not include rationale, transcript quotations, actions, referrals, consequences, severity, or workflow decisions.`
}
