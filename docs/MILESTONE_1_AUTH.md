...
The session cookie is `HttpOnly`, `SameSite=Lax`, path-scoped to `/`, host-only, and `Secure` in production. A standard session has a 12-hour hard absolute lifetime and an 8-hour idle timeout; qualifying authenticated activity refreshes only the idle deadline, never the absolute expiry. Before Cognito Managed Login, a person may explicitly select “Keep me signed in on this device for 30 days”. That creates a `trusted_device` application session with a hard, non-sliding 30-day absolute lifetime and no standard idle timeout. This is appropriate only for a private or individually assigned device protected by a screen lock or device sign-in; it must not be used on shared or public devices. Both modes are opaque PostgreSQL-backed application sessions, remain subject to current user/organisation/role checks on every request, and can be revoked server-side. A separate signed, short-lived, HttpOnly OIDC transaction cookie holds state, nonce, PKCE verifier, and the selected session mode. Neither cookie is available to JavaScript or `localStorage`.
...
For later enterprise deployment, customer workforce identity and device authentication should be federated through Cognito (for example, Microsoft Entra ID) rather than introducing a Te Kaupapa desktop password. Cognito Managed Login remains responsible for passkeys/WebAuthn and email-OTP fallback; Te Kaupapa does not retain Cognito refresh tokens or implement custom WebAuthn.

Ordinary Kaimahi workflow continuity is appropriate for the trusted-session policy. Before enabling or expanding higher-risk capability, consider a narrow recent-authentication boundary for Supervisor access, transcript/source-evidence access, Pou specification editing and approval/activation, organisation/user administration, and formal safety or governance actions. This is a deferred product/security decision; it does not change the current server-side role and organisation checks.
...
After that correction, Cognito accepted the lowercase verification preference but rejected the premature `WebAuthnRelyingPartyID` value because the managed-login prefix domain is created by the separate `AWS::Cognito::UserPoolDomain` resource. `WebAuthnRelyingPartyID` is optional in the CloudFormation resource, so the template no longer sets it while creating the pool; Cognito can apply its default while the subsequent domain resource establishes the prefix. The deployed pool's passkey relying-party configuration must be inspected after a successful stack creation. The failed protected pool and `ROLLBACK_FAILED` stack from this attempt are not altered without separate approval.

With that correction, the canonical stack created its tagged Cognito Essentials pool, server app client, and managed-login v2 prefix domain successfully. The pool has admin-only creation, `EMAIL_OTP`, `WEB_AUTHN`, and provider-required `PASSWORD` first factors, MFA off, and the verified SES sender; the client has the approved authorization-code callback and logout URLs. Cognito then denied the read-only `cognito-idp:GetUserPoolMfaConfig` call needed to inspect the effective RP ID and user-verification setting. The split Cognito/SES policy now adds only that action through the existing four-tag user-pool resource condition. No pilot user, provider interaction, or application session is attempted until an IAM administrator applies that correction.

After that read permission was applied, `GetUserPoolMfaConfig` confirmed MFA `OFF`, WebAuthn user verification `preferred`, and `SINGLE_FACTOR`, but returned no RP ID. The template now supports a two-stage, CloudFormation-managed passkey configuration: initial creation leaves `WebAuthnRelyingPartyId` empty so the prefix domain can be created, then an ordinary update supplies the exact active managed-login domain as the RP ID. This avoids the invalid pre-domain configuration while keeping the final setting in the stack template rather than as an untracked direct provider change.

Before the expressly approved update could be reviewed, CloudFormation denied `cloudformation:CreateChangeSet` on the successful canonical stack. The split CloudFormation policy now adds only `CreateChangeSet`, `DescribeChangeSet`, and `DeleteChangeSet` for the named `te-kaupapa-passkey-rpid-20260809` change set on that one immutable stack ARN, with the exact Cognito resource types and four Te Kaupapa stack/request tags. It grants no `ExecuteChangeSet`, stack deletion, or access to another stack. An IAM administrator must apply this correction before the required replacement assessment and RP-ID update can proceed.

After the policy was applied, CloudFormation created the approved named change set but its waiter was denied `DescribeChangeSet`: the AWS CLI supplies the generated change-set ARN, rather than only its short name, to the `cloudformation:ChangeSetName` IAM condition. The CloudFormation policy now adds only that already-created ARN to the exact-name condition; it does not add a wildcard or another stack/change-set permission. An IAM administrator must apply this small correction before the change set can be inspected. The stack has not been updated or executed.

After that generated-ARN correction was applied, CloudFormation still denied `DescribeChangeSet`. The API authorizes this read against the parent stack and does not satisfy the change-set-name/resource-tag conditions used by the prior statement. The policy now provides only read-only `DescribeChangeSet` on the one immutable successful stack ARN in `ap-southeast-2`; create and delete remain exact-name constrained, and execution is not permitted. An IAM administrator must apply this correction before the existing change set can be reviewed. The stack has not been updated or executed.

CloudFormation then described the existing change set successfully. It is `CREATE_COMPLETE` and `AVAILABLE`, contains exactly one `Modify` change, and retains pool `ap-southeast-2_gMr4TLs7r` with `Replacement=False`; its only property is `WebAuthnRelyingPartyID`, which never requires recreation. The app client and managed-login domain are absent from the changes, and no resource is added, removed, or replaced. After Peter explicitly approved execution of that exact reviewed change set, the local CloudFormation policy was narrowed to add only `ExecuteChangeSet` for its generated ARN and parent stack in `ap-southeast-2`; no other change set can be executed. An IAM administrator must apply that policy revision before execution. The change set has not yet been executed.

After the execution policy was applied, CloudFormation denied `ExecuteChangeSet` when the CLI supplied the generated ARN: the API evaluates `cloudformation:ChangeSetName` as its fixed short name. The policy now accepts only that short name or the already-reviewed ARN, still bound to the one immutable canonical stack. No execution or stack operation has started; an IAM administrator must apply this correction before retrying the exact approved change set.

With that execution correction applied, the reviewed change set completed successfully. The canonical stack is `UPDATE_COMPLETE`; the existing pool, client, and managed-login domain are preserved, deletion protection remains `ACTIVE`, and effective WebAuthn configuration is MFA `OFF`, `SINGLE_FACTOR`, user verification `preferred`, and relying-party ID `te-kaupapa-staging.auth.ap-southeast-2.amazoncognito.com`. SES still verifies `hello@talkscape.com` with DKIM `SUCCESS`, and the account has production sending enabled. The attempt to read Cognito's SES service-linked role was denied at `iam:GetRole`, so the split Cognito/SES policy now adds read-only access to only `arn:aws:iam::905418481310:role/aws-service-role/email.cognito-idp.amazonaws.com/AWSServiceRoleForAmazonCognitoIdpEmailService`. An IAM administrator must apply that correction before the role can be confirmed and temporary bootstrap permissions can be removed.

After that read permission was applied, the Cognito SES service-linked role was verified: its exact name and service-linked path are `AWSServiceRoleForAmazonCognitoIdpEmailService` and `/aws-service-role/email.cognito-idp.amazonaws.com/`, and its trust policy permits only `email.cognito-idp.amazonaws.com`. The temporary `iam:CreateServiceLinkedRole` and reviewed change-set `cloudformation:ExecuteChangeSet` permissions have now been removed locally. They must be removed from the two customer-managed AWS policies by an IAM administrator; no AWS IAM policy was modified by this repository change.

When the managed-login endpoint displayed `Login pages unavailable`, AWS documentation identified the likely cause: programmatically created app clients have no managed-login branding style until `CreateManagedLoginBranding` associates one. The first read-only confirmation call was denied at `cognito-idp:DescribeManagedLoginBrandingByClient` on the tagged Te Kaupapa pool. The Cognito/SES policy now adds only that action through its existing four-tag user-pool condition. An IAM administrator must apply this correction before the app client's branding can be confirmed or any branding resource is proposed.

After that read permission was applied, `DescribeManagedLoginBrandingByClient` returned `ResourceNotFoundException` for `te-kaupapa-server-staging`, confirming that no style is assigned and explaining why Cognito's Managed Login page is unavailable. The template now declares `TeKaupapaManagedLoginBranding` for the existing pool and server client with only `UseCognitoProvidedValues: true`; it supplies no assets, settings, CSS, colours, fonts, or Te Kaupapa UI changes. The Cognito/SES policy adds only `CreateManagedLoginBranding`, `DescribeManagedLoginBranding`, and `DeleteManagedLoginBranding` through the existing four-tag pool condition: create and read are required for CloudFormation creation/stabilization, and delete is needed only for automatic rollback. No branding update permission is granted. An IAM administrator must apply that correction before a change set can safely review the addition.
