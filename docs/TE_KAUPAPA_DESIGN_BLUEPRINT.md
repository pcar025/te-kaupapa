...
The same approved architecture applies to every Te Waharoa Pou: a source-derived immutable organisation specification produces a current-conversation-only guidance projection, a separate structured review projection, and a link to the distinct immutable safety projection. New workflows use the mandatory journey order `kaitiakitanga`, `tikanga`, `whakapapa`, `manaakitanga`, `puukenga`, `haepapa`, and `oranga`. Existing workflows retain their persisted checkpoint order so their semantic progression does not change retrospectively. The source is `src/imports/pasted_text/te-waharoa-model-update.md` (`#pou-1` through `#pou-7`), pinned by its raw SHA-256 in each derived version.
