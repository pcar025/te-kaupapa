...
- `cognito-idp`: create and apply the four required tags to the new user pool, then configure (including the template's explicit MFA-off setting), inspect, and roll back only tagged Te Kaupapa user pools; create/get/delete pilot users only in those pools. It also permits read-only availability checking for the one requested Cognito domain prefix.
