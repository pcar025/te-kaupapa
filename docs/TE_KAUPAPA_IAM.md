...
- `cloudformation`: additionally create, inspect, and delete one named change set only for the already-successful canonical authentication stack. This is limited to the approved passkey RP-ID review, the fixed resource types, the four required request/resource tags, and change-set name `te-kaupapa-passkey-rpid-20260809`; it cannot execute a change set.
