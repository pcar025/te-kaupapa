...
- `cloudformation`: additionally create, inspect, and delete one named review change set only for the already-successful canonical authentication stack. This is limited to the default Managed Login branding review, the four fixed resource types, the four required request/resource tags, and change-set name `te-kaupapa-managed-login-branding-20260810`; it cannot execute a change set.
...
The completed passkey RP-ID review exception has been replaced rather than retained: `CreateChangeSet` and `DeleteChangeSet` now apply only to stack ID `29a3c780-93e1-11f1-936a-02407fbd357b` and the fixed name `te-kaupapa-managed-login-branding-20260810`. Its permitted resource types add only `AWS::Cognito::ManagedLoginBranding`, required to preview Cognito's default branding resource. CloudFormation denied `DescribeChangeSet` when that action was conditioned on a generated change-set ARN. AWS authorizes that action against the parent stack ARN but does not reliably provide the change-set/tag condition context used by the prior statement, so it remains read-only `DescribeChangeSet` scoped only to that one immutable stack ARN and `ap-southeast-2`. No `ExecuteChangeSet` is granted; the new change set cannot be executed. The separately scoped `UpdateStack` action remains unused for this review.
