...
- `iam`: one `CreateServiceLinkedRole` exception, constrained to Cognito's `email.cognito-idp.amazonaws.com` service principal so Cognito can create its own SES email-delivery service-linked role when absent, plus read-only `GetRole` for that one resulting service-linked-role ARN to verify it before the creation permission is removed.
...
Neither replacement policy grants IAM administration, `iam:*`, `iam:PassRole`, RDS permissions, Secrets Manager permissions, SES identity/DNS/sending/production-access operations, stack-set operations, or broad service wildcards. The only IAM exceptions are the exact `iam:CreateServiceLinkedRole` action required by Cognito email configuration, constrained to `email.cognito-idp.amazonaws.com`, and read-only `iam:GetRole` for `AWSServiceRoleForAmazonCognitoIdpEmailService` at its one service-linked-role ARN. They cannot create a role for another service or modify/delete/attach any role. They also exclude public signup, password/SMS configuration work, deployment hosting, and all Milestone 2 application capabilities.
...
The one passkey change-set exception is narrower still: `CreateChangeSet` and `DeleteChangeSet` apply only to stack ID `29a3c780-93e1-11f1-936a-02407fbd357b` and the fixed name `te-kaupapa-passkey-rpid-20260809`. CloudFormation denied `DescribeChangeSet` even after the generated change-set ARN `.../bcf42fb5-683d-408b-a25f-3a7fc694923b` was added to the name condition. AWS authorizes that action against the parent stack ARN but does not reliably provide the change-set/tag condition context used by the prior statement, so its replacement is read-only `DescribeChangeSet` scoped only to that one immutable stack ARN and `ap-southeast-2`. Following Peter's review and explicit execution approval, `ExecuteChangeSet` is allowed only against that same stack and only when `cloudformation:ChangeSetName` is the reviewed generated ARN or its fixed short name. CloudFormation evaluates execution with the short name even when the CLI is supplied the ARN; together with the immutable parent-stack ARN, this resolves only the reviewed change set. It cannot execute a change set on another stack or any CareFlow resource. The separately scoped `UpdateStack` action remains unused for this change.
